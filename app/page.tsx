...
const PERSONALIZATION_SLIDE_INTERVAL = 3000;
...
function Brand({ storeName = 'Meu Vício' }: { storeName?: string }) {
  return <a href="#inicio" className="flex items-center gap-3" aria-label={`${storeName} - início`}><span className="brand-mark">mv</span><span><span className="block font-[var(--font-display)] text-[17px] font-bold tracking-[0.14em] text-[#1e1e1e]">{storeName.toUpperCase()}</span><span className="block text-[9px] font-semibold uppercase tracking-[0.22em] text-[#d71920]">calçados & acessórios</span></span></a>;
}

function StoreHeader({ storeName, mobileMenu, setMobileMenu, search, setSearch, setCategory, categories, selectedCategory, cartCount, setCartOpen, favoritesCount, showFavoritesOnly, setShowFavoritesOnly }: { storeName: string; mobileMenu: boolean; setMobileMenu: (open: boolean) => void; search: string; setSearch: (value: string) => void; setCategory: (category: string) => void; categories: string[]; selectedCategory: string; cartCount: number; setCartOpen: (open: boolean) => void; favoritesCount: number; showFavoritesOnly: boolean; setShowFavoritesOnly: (show: boolean) => void }) {
  return <>
    ...
    <header className="site-header">
      <div className="site-header-main mx-auto flex max-w-[1240px] items-center justify-between px-5 py-3 lg:px-8">
        ...
        <div className="flex items-center gap-2"><button className={showFavoritesOnly ? 'icon-button favorite-toggle active' : 'icon-button favorite-toggle'} type="button" onClick={() => { setShowFavoritesOnly(!showFavoritesOnly); document.getElementById('colecao')?.scrollIntoView({ behavior: 'smooth' }); }} aria-pressed={showFavoritesOnly} aria-label={showFavoritesOnly ? 'Mostrar todos os produtos' : 'Ver favoritos'} title={showFavoritesOnly ? 'Mostrar todos os produtos' : 'Ver favoritos'}><Heart size={19} strokeWidth={1.8} fill={showFavoritesOnly ? 'currentColor' : 'none'} />{favoritesCount > 0 && <span className="favorite-count">{favoritesCount}</span>}</button><button className="cart-button" type="button" onClick={() => setCartOpen(true)}><ShoppingBag size={18} strokeWidth={2} /><span className="hidden sm:inline">Sacola</span><span className="cart-count">{cartCount}</span></button></div>
      </div>
      <div className="site-tools mx-auto max-w-[1240px] px-5 pb-3 lg:px-8"><label className="site-search"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="O que você está procurando?" aria-label="Buscar no catálogo" /></label><nav className="category-links" aria-label="Categorias"><a className={selectedCategory === 'Todos' ? 'active' : ''} href="#colecao" onClick={() => setCategory('Todos')}>Todos</a>{categories.map((name) => <a className={selectedCategory === name ? 'active' : ''} href="#colecao" key={name} onClick={() => setCategory(name)}>{name}</a>)}<a className="category-special" href="#colecao" onClick={() => setCategory('Todos')}>Mais vendidos</a><a className="category-special" href="#como-funciona">Personalizados</a></nav></div>
    </header>
  </>;
}
...
export default function Home() {
  ...
  const [catalogCategories, setCatalogCategories] = useState<string[]>(['Adulto', 'Infantil']);
  ...
  useEffect(() => {
    ...
    async function loadCatalog() {
      ...
      const [{ data }, { data: categoryRows }] = await Promise.all([
        supabase.from('products').select('id,name,category,color,tag,price,description,image_url,product_sizes(size,quantity)').eq('is_active', true).order('created_at', { ascending: false }),
        supabase.from('categories').select('id,name').order('name', { ascending: true }),
      ]);
      if (!mounted) return;
      const nextProducts = (data || []).map((row) => { const image = parseImageUrl(row.image_url); return { id: row.id, name: row.name, category: String(row.category || 'Adulto'), color: row.color || 'Sem cor', tag: row.tag || undefined, price: Number(row.price), description: row.description || 'Confira os detalhes deste modelo.', image: image.src, imageAdjust: image.adjust, sizes: Object.fromEntries((row.product_sizes || []).map((item: { size: number; quantity: number }) => [item.size, item.quantity])) }; });
      setCatalogProducts(nextProducts);
      const available = new Set([...(categoryRows || []).map((row: { name?: string }) => String(row.name || '').trim()), ...nextProducts.map((product) => product.category)]);
      setCatalogCategories(['Adulto', 'Infantil', ...Array.from(available).filter((name) => name && name !== 'Adulto' && name !== 'Infantil').sort((a, b) => a.localeCompare(b))]);
  ...
  return <main className="site-shell min-h-screen overflow-x-hidden bg-white text-[#1e1e1e]">
    <StoreHeader storeName={settings.storeName} mobileMenu={mobileMenu} setMobileMenu={setMobileMenu} search={search} setSearch={setSearch} setCategory={setCategory} categories={catalogCategories} selectedCategory={category} cartCount={cartCount} setCartOpen={setCartOpen} favoritesCount={favorites.length} showFavoritesOnly={showFavoritesOnly} setShowFavoritesOnly={setShowFavoritesOnly} />
    <section id="inicio" className="catalog-hero"><div className="mx-auto grid max-w-[1240px] items-center gap-10 px-5 py-12 lg:grid-cols-[1.08fr_.92fr] lg:px-8 lg:py-20"><div className="relative z-10"><div className="hero-carousel" aria-label="Destaques da loja" aria-roledescription="carousel" aria-live="polite"><div className="hero-carousel-topline"><span>deslize pelos destaques</span><div className="hero-carousel-arrows"><button type="button" aria-label="Destaque anterior" onClick={() => setHeroSlide((current) => (current + 1) % 2)}><ArrowLeft size={15} /></button><button type="button" aria-label="Próximo destaque" onClick={() => setHeroSlide((current) => (current + 1) % 2)}><ArrowRight size={15} /></button></div></div>{heroSlide === 0 && <div className="hero-slide"><p className="eyebrow"><Sparkles size={14} /> personalização</p><h1>Monte seu par do seu <em>jeito.</em></h1><p className="hero-copy">A loja também trabalha com Havaianas personalizadas. Consulte os modelos e as opções disponíveis.</p><div className="mt-7 flex flex-wrap gap-3"><a className="primary-button" href={'https://wa.me/' + settings.whatsappPrimary + '?text=' + encodeURIComponent('Olá! Gostaria de saber sobre as Havaianas personalizadas.')} target="_blank" rel="noreferrer">Consultar no WhatsApp <MessageCircle size={17} /></a><a className="secondary-button" href="#colecao">Ver produtos</a></div></div>}{heroSlide === 1 && <div className="hero-slide"><p className="eyebrow"><Sparkles size={14} /> cartão fidelidade</p><h1>A cada 10 Havaianas, <em>ganhe 1.</em></h1><p className="hero-copy">Junte suas compras, acompanhe sua fidelidade e aproveite uma Havaiana de presente ao completar 10 unidades.</p><div className="mt-7 flex flex-wrap gap-3"><a className="primary-button" href="#colecao">Começar a juntar <ArrowRight size={17} /></a><a className="secondary-button" href="#contato">Falar com a loja</a></div></div>}</div></div>{heroSlide === 0 ? <div className="hero-story-card hero-personalization-card" role="group" aria-roledescription="carousel" aria-label="Imagens de personalização"><img key={personalizationImages[personalizationImage].src} src={personalizationImages[personalizationImage].src} alt={personalizationImages[personalizationImage].alt} /><div className="hero-story-overlay"><span>personalize do seu jeito:</span><strong>Escolha os melhores pingentes<br />para personalizar seu par.</strong></div><button className="hero-story-arrow previous" type="button" aria-label="Imagem anterior de personalização" onClick={() => setPersonalizationImage((current) => (current + personalizationImages.length - 1) % personalizationImages.length)}><ArrowLeft size={16} /></button><button className="hero-story-arrow next" type="button" aria-label="Próxima imagem de personalização" onClick={() => setPersonalizationImage((current) => (current + 1) % personalizationImages.length)}><ArrowRight size={16} /></button><div className="hero-story-dots" role="tablist" aria-label="Fotos de personalização">{personalizationImages.map((image, index) => <button key={image.src} className={personalizationImage === index ? 'active' : ''} type="button" role="tab" aria-label={`Mostrar imagem ${index + 1}`} aria-selected={personalizationImage === index} onClick={() => setPersonalizationImage(index)} />)}</div></div> : <div className="hero-story-card hero-loyalty-card"><img src="/products/hero-fidelidade-v2.png" alt="Chinelos coloridos reunidos em uma composição de fidelidade" /><div className="hero-story-overlay"><span>benefício para clientes</span><strong>Volte sempre.<br />A próxima conquista é sua.</strong></div></div>}<div className="hero-carousel-support">{heroSlide === 0 ? <div className="hero-notes"><span><Check size={15} /> Atendimento personalizado</span><span><Check size={15} /> Consulte as opções</span></div> : <div className="hero-notes"><span><Check size={15} /> Benefício para clientes</span><span><Check size={15} /> Consulte as regras na loja</span></div>}<div className="hero-carousel-progress" aria-hidden="true"><span style={{ width: ((heroSlide + 1) / 2) * 100 + '%' }} /></div><div className="hero-carousel-tabs" role="tablist" aria-label="Destaques"><button className={heroSlide === 0 ? 'hero-carousel-tab active' : 'hero-carousel-tab'} type="button" role="tab" aria-selected={heroSlide === 0} onClick={() => setHeroSlide(0)}><span>01</span> Personalização</button><button className={heroSlide === 1 ? 'hero-carousel-tab active' : 'hero-carousel-tab'} type="button" role="tab" aria-selected={heroSlide === 1} onClick={() => setHeroSlide(1)}><span>02</span> Fidelidade</button></div></div></div></section>
    <section id="colecao" className="mx-auto max-w-[1240px] px-5 py-10 lg:px-8 lg:py-16"><div className="section-heading"><div><p className="eyebrow warm">nossa seleção</p><h2>{showFavoritesOnly ? 'Seus favoritos' : 'Escolha pelo seu tamanho'}</h2></div><p>{showFavoritesOnly ? 'Os modelos que você salvou ficam disponíveis aqui neste aparelho.' : 'Filtre por faixa de numeração ou escolha entre adulto e infantil.'}</p></div><div className="filters-panel"><div className="filter-search"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar modelo ou cor" aria-label="Buscar modelo ou cor" /></div><label className="category-select"><span>Categoria</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option>Todos</option>{catalogCategories.map((name) => <option key={name}>{name}</option>)}</select><ChevronDown size={16} /></label><div className="size-filter-row"><div className="filter-label"><Filter size={16} /> Tamanho</div><div className="size-list" aria-label="Filtrar por tamanho"><button className={selectedSize === null ? 'size-chip active' : 'size-chip'} onClick={() => { setSelectedSize(null); setCategory('Todos'); }} type="button">Todos</button>{visibleSizeOptions.map((option) => <button key={option.label} className={selectedSize === option.label ? 'size-chip active' : 'size-chip'} onClick={() => setSelectedSize(option.label)} type="button">{option.label}</button>)}</div></div></div><div className="results-row"><span><strong>{catalogLoading ? '—' : filteredProducts.length}</strong> modelos encontrados</span><span className="results-actions">{showFavoritesOnly && <button type="button" onClick={() => setShowFavoritesOnly(false)}>Ver todos <X size={14} /></button>}{selectedSize && <button type="button" onClick={() => setSelectedSize(null)}>Limpar tamanho <X size={14} /></button>}</span></div>{catalogLoading ? <div className="empty-state"><span><Search size={21} /></span><h3>Carregando catálogo...</h3><p>Buscando os produtos cadastrados pela loja.</p></div> : filteredProducts.length ? <div className="product-grid">{filteredProducts.map((product) => { const availableSizes = (product.category === 'Adulto' ? adultSizeOptions : product.category === 'Infantil' ? infantSizeOptions : sizeOptions).filter((option) => option.values.some((value) => (product.sizes[value] ?? 0) > 0)); const isFavorite = favorites.includes(product.id); return <article key={product.id} className="product-card"><div className={`product-image ${product.color.toLowerCase().replace(' ', '-')}`}>{product.tag && <span className="product-tag">{product.tag}</span>}<button className={isFavorite ? 'product-heart active' : 'product-heart'} type="button" aria-pressed={isFavorite} aria-label={isFavorite ? `Remover ${product.name} dos favoritos` : `Favoritar ${product.name}`} onClick={() => toggleFavorite(product.id)}><Heart size={17} fill={isFavorite ? 'currentColor' : 'none'} /></button><button className="product-image-open" type="button" onClick={() => setImagePreviewProduct(product)} aria-label={`Abrir foto completa de ${product.name}`}><img style={imageAdjustStyle(product.imageAdjust)} src={product.image} alt={`${product.name} - ${product.color}`} /></button><span className="product-color-dot" title={product.color} /></div><div className="product-info"><div className="flex items-start justify-between gap-3"><div><span className="product-category">Havaianas {product.category.toLowerCase()} <span>•</span> {product.color}</span><h3>{product.name}</h3></div><strong className="product-price">{money(product.price)}</strong></div><p>{product.description}</p><div className="available-row"><span>Tamanhos disponíveis</span><div className="available-sizes">{availableSizes.slice(0, 6).map((option) => <span key={option.label}>{option.label}</span>)}{availableSizes.length > 6 && <span>+{availableSizes.length - 6}</span>}</div></div><button className="choose-button" type="button" onClick={() => setActiveProduct(product)}>Escolher tamanho <ArrowRight size={16} /></button></div></article>; })}</div> : <div className="empty-state"><span><Heart size={21} /></span><h3>{showFavoritesOnly ? 'Você ainda não salvou favoritos' : 'Nenhum modelo encontrado'}</h3><p>{showFavoritesOnly ? 'Toque no coração de um produto para encontrá-lo rapidamente depois.' : 'Experimente outra numeração ou limpe os filtros.'}</p><button className="secondary-button" type="button" onClick={() => { setSelectedSize(null); setSearch(''); setShowFavoritesOnly(false); }}>Ver todos os produtos</button></div>}</section>
