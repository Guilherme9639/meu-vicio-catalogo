...
export default function AdminPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'Adulto', color: '', tag: '', price: '', description: '', isActive: true, sizes: [] as string[], quantities: {} as Record<string, number>, image: '/products/havaianas-branco.png' });
  useEffect(() => {
  ...
  async function saveProduct() {
    ...
    if (supabase && session && editingProductId && !editingProductId.startsWith('demo-')) {
      const update = await supabase.from('products').update({ name: form.name.trim(), category: form.category, color: form.color.trim(), tag: form.tag || null, price: numericPrice, description: form.description.trim(), image_url: imageUrl, is_active: form.isActive }).eq('id', editingProductId);
