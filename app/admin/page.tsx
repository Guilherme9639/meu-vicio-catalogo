...
type AdminProduct = { id: string; name: string; category: string; color?: string; price: string; stock: number; status: 'Ativo' | 'Rascunho'; sizes: string[]; sizeQuantities?: Record<string, number>; description?: string; image: string };
...
export default function AdminPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'Adulto', color: '', price: '', description: '', isActive: true, sizes: [] as string[], quantities: {} as Record<string, number>, image: '/products/havaianas-branco.png' });
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      const { data } = await supabase.from('products').select('id,name,category,color,price,description,is_active,image_url,product_sizes(size,quantity)').order('created_at', { ascending: false });
  ...
  const visibleOrders = useMemo(() => { const term = orderSearch.trim().toLowerCase(); if (!term) return orders; return orders.filter((order) => `${order.id} ${order.whatsappNumber || ''} ${order.items.map((item) => `${item.productName} ${item.size}`).join(' ')}`.toLowerCase().includes(term)); }, [orders, orderSearch]);
  ...
  async function saveProduct() {
    ...
    if (supabase && session && editingProductId && !editingProductId.startsWith('demo-')) {
      const update = await supabase.from('products').update({ name: form.name.trim(), category: form.category, color: form.color.trim(), price: numericPrice, description: form.description.trim(), image_url: imageUrl, is_active: form.isActive }).eq('id', editingProductId);
  ...
  async function saveSettings(event: FormEvent) {
    ...
    const { error } = await supabase.from('store_settings').upsert({ id: 'default', store_name: settings.storeName.trim() || 'Meu Vício', hours: settings.hours.trim(), whatsapp_primary: settings.whatsappPrimary.replace(/\D/g, ''), whatsapp_secondary: settings.whatsappSecondary.replace(/\D/g, ''), whatsapp_secondary_label: settings.whatsappSecondaryLabel.trim(), whatsapp_message: settings.whatsappMessage.trim() }, { onConflict: 'id' });
    ...
    <section className="admin-content"><header className="admin-topbar"><div><span className="admin-breadcrumb">Painel / Visão geral</span><h1>{greeting}, {displayName} <span>✦</span></h1></div><div className="admin-top-actions"><button type="button" aria-label="Notificações indisponíveis" title="Notificações em breve" disabled><Bell size={19} /></button><a className="view-store" href="/" target="_blank" rel="noreferrer">Ver catálogo <ArrowLeft size={15} /></a></div></header><nav className="admin-mobile-nav" aria-label="Seções do painel"><a href="#visao-geral">Visão geral</a><a href="#produtos">Produtos</a><a href="#categorias">Categorias</a><a href="#pedidos">Pedidos</a><a href="#configuracoes">Preferências</a></nav>
      <div className="admin-main" id="visao-geral">{actionMessage && <div className="admin-action-message" role="status"><Check size={16} /> <span>{actionMessage}</span><button type="button" onClick={() => setActionMessage('')} aria-label="Fechar mensagem"><X size={16} /></button></div>}{importNotice && <div className="import-notice"><div><span className="notice-icon"><Check size={16} /></span><div><strong>Drive usado somente para a primeira carga</strong><p>Depois da importação, novos produtos, fotos e estoques serão atualizados diretamente neste painel.</p></div></div><button type="button" onClick={() => setImportNotice(false)} aria-label="Fechar aviso"><X size={17} /></button></div>}
        ...
        <section id="configuracoes" className="admin-settings-section">
          ...
          <form className="admin-settings-form" onSubmit={saveSettings}><div className="admin-settings-grid"><label className="admin-settings-card"><span>Nome da loja</span><input value={settings.storeName} onChange={(event) => setSettings({ ...settings, storeName: event.target.value })} /></label><label className="admin-settings-card"><span>Horário de atendimento</span><input value={settings.hours} onChange={(event) => setSettings({ ...settings, hours: event.target.value })} /></label><label className="admin-settings-card"><span>WhatsApp principal</span><input value={settings.whatsappPrimary} onChange={(event) => setSettings({ ...settings, whatsappPrimary: event.target.value })} inputMode="numeric" /></label><label className="admin-settings-card"><span>WhatsApp secundário</span><input value={settings.whatsappSecondary} onChange={(event) => setSettings({ ...settings, whatsappSecondary: event.target.value })} inputMode="numeric" /></label><label className="admin-settings-card"><span>Identificação do segundo atendimento</span><input value={settings.whatsappSecondaryLabel} onChange={(event) => setSettings({ ...settings, whatsappSecondaryLabel: event.target.value })} /></label><label className="admin-settings-card"><span>Mensagem inicial do pedido</span><textarea value={settings.whatsappMessage} onChange={(event) => setSettings({ ...settings, whatsappMessage: event.target.value })} rows={2} /></label></div><div className="admin-settings-footer"><small>O Drive foi usado apenas como apoio na carga inicial. O catálogo usa os dados salvos no sistema.</small><button type="submit" className="new-product-button" disabled={settingsSaving}>{settingsSaving ? 'Salvando...' : 'Salvar preferências'} <Check size={16} /></button></div></form>
        </section>
      </div></section>
